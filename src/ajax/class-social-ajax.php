...
class Social_Ajax
{
    ...
    public function init_bind_social(): void
    {
        ...
        if (!Helper::is_social_login_enabled()) {
            wp_send_json_error(['message' => __('Social login is not enabled.', 'swift-login')], 403);
        }
        ...
        if (empty($appid) || empty($appkey)) {
            wp_send_json_error(['message' => __('Social login is not configured. Please contact the administrator.', 'swift-login')], 500);
        }
        ...
        if (is_wp_error($response)) {
            wp_send_json_error(['message' => __('Could not connect to the login service.', 'swift-login')], 502);
        }

        $body = json_decode(wp_remote_retrieve_body($response), true);
        if (!isset($body['code']) || $body['code'] !== 0) {
            $msg = $body['msg'] ?? __('Failed to get bind URL.', 'swift-login');
    ...
    public function unbind_social(): void
    {
        ...
        wp_send_json_success(['message' => __('Account unlinked successfully.', 'swift-login')]);
    }
    ...
    public function init_social_login(): void
    {
        ...
        $type = sanitize_key($_POST['type'] ?? '');
        if (empty($type)) {
            wp_send_json_error(['message' => __('Missing login platform parameter.', 'swift-login')], 400);
        }

        $allowed = Helper::get_social_platforms();
        if (!in_array($type, $allowed, true)) {
            wp_send_json_error(['message' => __('Unsupported login platform.', 'swift-login')], 400);
        }
        ...
        if (!isset($body['code']) || $body['code'] !== 0) {
            $msg = $body['msg'] ?? __('Failed to get login URL.', 'swift-login');
            wp_send_json_error(['message' => $msg], 500);
        }
