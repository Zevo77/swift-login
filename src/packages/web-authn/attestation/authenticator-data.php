...
namespace Swift_Login\Packages\Web_Authn\Attestation;

use Swift_Login\Packages\Web_Authn\Web_Authn_Exception;
use Swift_Login\Packages\Web_Authn\Cbor\Cbor_Decoder;
...
class Authenticator_Data
{
    ...
    public function getPublicKeyPem()
    {
        ...
        switch ($this->_attestedCredentialData->credentialPublicKey->kty ?? null) {
            ...
            case self::$_RSA_TYPE: $der = $this->_getRsaDer();
                break;
            case self::$_OKP_TYPE: $der = $this->_getOkpDer();
                break;
            default: throw new Web_Authn_Exception('invalid key type', Web_Authn_Exception::INVALID_DATA);
        }
    ...
    {
        $enc = Cbor_Decoder::decodeInPlace($binary, $offset, $endOffset);
        ...
        switch ($credPKey->alg) {
            ...
            case self::$_RSA_RS256: $this->_readCredentialPublicKeyRS256($credPKey, $enc);
                break;
            case self::$_OKP_EDDSA: $this->_readCredentialPublicKeyEDDSA($credPKey, $enc);
                break;
        }
    ...
    private function _readCredentialPublicKeyEDDSA(&$credPKey, $enc)
    {
        ...
        $credPKey->x   = $enc[self::$_COSE_X] instanceof Byte_Buffer ? $enc[self::$_COSE_X]->getBinaryString() : null;
        unset($enc);
    ...
    private function _readCredentialPublicKeyES256(&$credPKey, $enc)
    {
        $credPKey->crv = $enc[self::$_COSE_CRV];
        $credPKey->x   = $enc[self::$_COSE_X] instanceof Byte_Buffer ? $enc[self::$_COSE_X]->getBinaryString() : null;
        $credPKey->y   = $enc[self::$_COSE_Y] instanceof Byte_Buffer ? $enc[self::$_COSE_Y]->getBinaryString() : null;
        unset($enc);
    ...
    {
        $credPKey->n = $enc[self::$_COSE_N] instanceof Byte_Buffer ? $enc[self::$_COSE_N]->getBinaryString() : null;
        $credPKey->e = $enc[self::$_COSE_E] instanceof Byte_Buffer ? $enc[self::$_COSE_E]->getBinaryString() : null;
        unset($enc);
